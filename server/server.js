...
const corsOptions = {
  origin: (origin, callback) => {
    ...
    if (uniqueAllowedOrigins.length === 0) {
      if (isDevelopment) {
        console.log(
          "⚠️  CORS: No origin list configured. Allowing all origins (dev mode)",
        );
        return callback(null, true);
      }
...
const server = app.listen(PORT, () => {
  ...
  console.log(
    `🔐 CORS whitelist: ${uniqueAllowedOrigins.length > 0 ? JSON.stringify(uniqueAllowedOrigins) : "All origins (dev mode)"}`,
  );
});
